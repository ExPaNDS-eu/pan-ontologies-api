      ...
      () => {
        it("A generator with a format that can be used by LoopbackCache",
          (done) => {
            ...
            const expected = [
              {
                ...
                relatives: ["2", "3"], createdAt: 1
              },
              {
                pid: "2", name: "b", synonym: ["B"],
                relatives: ["3", "4"], createdAt: 1
              }
            ];
            ...
            const results = [];
            for (var result of techniqueCache.prepareForCache(args))
              results.push(result);
      ...
      context(
        ...
        () => {
          it(`${message}`,
            (done) => {
              expect(
                techniqueCache.createSynonym(args)).to.be.eql(
                expected);
              done();
            });
        }
    ...
    context(
      ...
      () => {
        ...
        (done) => {
          ...
          const mock = sandbox.stub(techniqueCache.cache,
            "get").resolves([
      ...
      () => {
        ...
        (done) => {
          ...
          const mock = sandbox.stub(
            techniqueCache.cache, "get").onCall(0).resolves([
            { relatives: ["1", "2"], name: "a" } as Technique,
          ...
          techniqueCache.andOr(args, "and").then(
            data => {
              expect(data).to.be.eql(expected);
              expect(mock.getCall(0).calledWith({where: {
                or: [
                  { name: "a", pid: 1 } as Where<Technique>,
                  { synonym: "a", pid: 1 } as Where<Technique>
                ]
              }})).to.true;
              expect(mock.getCall(1).calledWith({where: { pid: "2" } as Where<Technique>})).to.true;
            });
          done();
        }
        );
        ...
        (done) => {
          ...
          techniqueCache.andOr(args, "or").then(
            data => {
              ...
              expect(mock.getCall(1).calledWith({where: { pid: { eq:"2" } } as Where<Technique>})).to.true;
            });
          done();
        }
        );
          ...
          it(`${message}`,
            (done) => {
              const mock = sandbox.stub(techniqueCache,
                expected === "flat"? expected: "andOr").resolves();
        ...
        it("Array of objects from filtered LoopbacCache or BioPortal",
          (done) => {
            ...
            techniqueCache.techniqueGetter.collection = [{
              pid: "1", prefLabel: "a", synonym: ["A"], parents: ["b"]
            }];
            const expected = {
              pid: "1", name: "a", synonym: ["A"],
            ...
            techniqueCache.buildTechniques().then(
              () => {
                expect((mock.args[0][1] as Generator<Technique>).next().value
                ).to.be.eql(expected);
              }
            );
            done();
